...
bool ti_values_initialized(void)
{
   return CLEAR_STR != NULL;
}
...
/**
 * @brief Replacement vdprintf when the library vdprintf is not available
 */
...
int vdprintf(int fd, const char *restrict format, va_list ap)
{
   ...
   int len = vsnprintf(NULL, 0, format, cargs);
   if (len > 0)
   {
      ...
      va_end(cargs);
      va_copy(cargs, ap);
      vsn_printf(buff, len+1, format, cargs);
      write(fd, buff, len);
   }
 ...
 *
 * This function uses vdprintf() for an unbuffered write
 * of the content.
 */
EXPORT int ti_printf(const char *fmt, ...)
{
   va_list args;
   va_start(args, fmt);
   int len = vdprintf(STDOUT_FILENO, fmt, args);
   va_end(args);
